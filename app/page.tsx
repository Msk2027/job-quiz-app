...
export default function Home() {
  ...
  const finishQuiz = () => {
    ...
    const newHistory = [newHistoryItem, ...history].slice(0, 100); 
        ...
        {gameState === 'menu' && (
          <div className="text-center w-full">
            ...
            <div className="space-y-3">
              <button 
                onClick={startQuiz}
                className="w-full bg-blue-600 text-white text-xl font-bold py-4 rounded-full shadow-lg hover:bg-blue-700 transition active:scale-[0.98]"
              >
                問題を始める
              </button>
              ...
              <button 
                ...
                className="w-full bg-white text-blue-600 border-2 border-blue-600 text-lg font-bold py-3 rounded-full hover:bg-blue-50 transition"
              >
                成績履歴を見る
              </button>
            </div>
            ...
            <div className="flex-1 overflow-y-auto max-h-[400px] pr-2 space-y-2">
              {history.length === 0 ? (
                <div className="text-center py-10 text-gray-400">
                  <p>まだ履歴がありません。<br/>問題を解くとここに表示されます。</p>
                </div>
              ) : (
                history.map((item, idx) => (
                  <div key={idx} className="flex justify-between items-center bg-gray-50 p-4 rounded border border-gray-100">
                    <div>
                      <div className="text-xs text-gray-500 mb-1">{item.date}</div>
                      <div className="font-bold text-gray-700">{item.mode}コース</div>
                    </div>
            ...
            <div className="grid gap-4 mb-6">
              {[currentQ.option1, currentQ.option2, currentQ.option3, currentQ.option4].map((option, index) => {
                ...
                if (isCorrect !== null) {
                   ...
                   if (index === correctNum) btnClass = "bg-green-100 border-green-400 text-green-800 font-bold";
                   else if (index !== correctNum && isCorrect === false) btnClass = "opacity-50";
                }
                return (
                  <button key={index} onClick={() => handleAnswer(index)} disabled={isCorrect !== null} className={`p-4 text-left border-2 rounded-lg transition-all ${btnClass}`}>
                    <span className="inline-block w-6 h-6 bg-gray-200 text-gray-600 text-center leading-6 rounded-full text-xs mr-3">{index + 1}</span>
                    {option}
                  </button>
                );
              })}
